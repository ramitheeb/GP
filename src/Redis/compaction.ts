...
import { Aggregation, AggregationType, Label, RedisTimeSeriesFactory } from "redis-time-series-ts";
import { RedisTimeSeries } from "redis-time-series-ts/lib/redisTimeSeries";

const createTimeSeriesRule=async (client:RedisTimeSeries,srcKey:string,dstKey:string,timeBucket:number)=>{

    // await client.create(key,labels,1800000);
    await client.createRule(srcKey,dstKey,new Aggregation(AggregationType.AVG,timeBucket));
}
